...
import { db, initDbSchema, closeDbClient, reinitDb } from "./src/db/index.ts";
...
app.get("/api/workplan", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    // Ensure default monthly plans exist if table is empty
    if (months.length === 0) {
      const defaultMonths = [
        { id: "2026-06", userId: dbUser.id, name: "June 2026", nameKh: "មិថុនា ២០២៦", status: "COMPLETED" },
        { id: "2026-07", userId: dbUser.id, name: "July 2026", nameKh: "កក្កដា ២០២៦", status: "IN_PROGRESS" },
        { id: "2026-08", userId: dbUser.id, name: "August 2026", nameKh: "សីហា 2026", status: "IN_PROGRESS" },
      ];
      await db.insert(monthlyPlans).values(defaultMonths).catch(() => {});

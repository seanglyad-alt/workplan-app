...
export default function WorkPlan({ 
  ...
  const fetchWorkPlanData = async () => {
    ...
    try {
      ...
      // Select active month if none is loaded
                ...
                {selectedTab === "calendar" && (
                  <div className="bg-[#111115] border border-white/[0.06] rounded-2xl p-5 shadow-inner overflow-x-auto">
                    <div className="min-w-[800px] space-y-4">
                      ...
                      <div className="grid grid-cols-[110px_repeat(7,_1fr)] gap-2 text-center pb-2 border-b border-white/[0.04]">
                        <div className="text-[10px] font-black text-slate-500 uppercase tracking-wider flex items-center justify-center">
                          Time Slot
                        </div>
                        {daysOfWeek.map((day, dIdx) => (
                          <div key={day.key} className="space-y-0.5 text-slate-300 font-sans p-1">
                            <span className="text-[11px] font-bold block">{day.kh.split(" ")[0]}</span>
                            <span className="text-[9px] font-mono text-slate-500 block">
                              {getWeekRangeLabel(selectedWeek).dates[dIdx] || "00/00/0000"}
                            </span>
                          </div>
                        ))}
                      </div>
